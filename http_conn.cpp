...
const char *error_401_form = "Unauthorized: Incorrect password\n";
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    *m_version++ = '\0';
    ...
    m_querys = strchr(m_url, '?');
    if (m_querys)
    {
        *m_querys++ = '\0';
        HTTP_CODE ret = parse_querys(m_querys);
        if (ret == BAD_REQUEST)
            return ret;
    }
    //printf("m_url: %s  m_querys: %s\n",m_url,m_querys);
    m_check_state = CHECK_STATE_HEADER;
    return NO_REQUEST;
}

// 解析HTTP请求的query信息
http_conn::HTTP_CODE http_conn::parse_querys(char *query_text)
{
    char *next_query;
    char *cur_query = query_text;
    char *key, *value;
    while (cur_query)
    {
        next_query = strchr(cur_query, '&');
        if (next_query)
        {
            *next_query++ = '\0';
        }
        key = strtok(cur_query, "=");
        value = strtok(NULL, "\0");
        if (value == NULL)
        {
            return BAD_REQUEST;
...
{
    //printf("%s",m_read_buf);
    if (strcasecmp(m_read_buf, "ping") == 0)
    {
        return PING;
    }
...
// 当得到一个完整、正确的HTTP请求时，交给router
http_conn::HTTP_CODE http_conn::do_request()
{
    return (m_router->handle(this));
}
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        {
            if (errno != EAGAIN)
            // 如果TCP写缓冲没有空间，则等待下一轮EPOLL1OUT事件，虽然在此期间，
        ...
        while (temp > 0)
        {
            if (m_iv[iv_n].iov_len > temp)
            {
                m_iv[iv_n].iov_len -= temp;
                m_iv[iv_n].iov_base += temp;
                temp = 0;
            }
            else
            {
                temp -= m_iv[iv_n].iov_len;
                m_iv[iv_n].iov_len = 0;
                iv_n++;
            }
        }
